...
public sealed class MovementMapper : IMapper<InventoryMovement, MovementData>
{
    ...
    => new(data.ProductId, data.Quantity, data.UnitCost, data.MovementType, data.Party, data.TransactionId){ Key = data.Id}; 
    public MovementData ToModel(InventoryMovement domain)
    => new(domain.Key, domain.ProductId, domain.Quantity, domain.UnitCost, domain.MovementType, domain.Party, domain.TransactionId);    
}